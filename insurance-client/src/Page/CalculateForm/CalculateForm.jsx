...
const CalculateForm = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      axios
      .get("HouseRiskCoefficien")
      .then((response) => {
        setRiskCoefficients(response.data.data);
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  return (
    <Grid container justifyContent="center">
      ...
      <Grid item xs={12} md={9} style={{ marginTop: "80px", marginBottom: "80px" }}>
        <TabContext value={tabValue}>
          <TabList onChange={handleChangeTab} aria-label="calculate tabs">
          ...
          <TabPanel sx={{ p: 0 }} value="life-insurance">
            <LifeInsuranceForm
              allWorkplace={workplaces}
              allDeathRate={deathRates}
            />
          </TabPanel>
          ...
          <TabPanel value="car-insurance" sx={{ p: 0 }}>
            <VehicleInsuranceForm
              allVehicleProperty={vehicleProperties}
              allVehicleType={vehicleTypes}
            />
          </TabPanel>
