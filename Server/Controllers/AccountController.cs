    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> ConfirmAccountEmail(string emailValidationToken, int userId)
        {
            ...
            if (!emailConfirmation)
                return BadRequest(new InsuranceErrorResponse(nameof(InvalidRequestException), "Bad email validation token or user Id."));
            return Ok();
        }
        ...
        public async Task<IActionResult> SignIn(AccountLoginDto accountLogin)
        {
            if (!await _userService.SignIn(accountLogin))
                return BadRequest(new InsuranceErrorResponse(nameof(InvalidRequestException), "Bad username or password."));

            var user = await _userService.GetAccount(accountLogin.UserName);
            if (!await _userService.EmailIsConfirmed(user.Id))
                return BadRequest(new InsuranceErrorResponse(nameof(InvalidRequestException), "Email must be confirmed before you can sign in."));
